...
class StatsOverviewWidget extends BaseWidget
{
    protected function getStats(): array
    {
        ...
        $testsCount = SiteTest::whereHas('site', function ($query) use ($userId) {
            $query->where('user_id', $userId);
        })->count();
        
        // Количество выполненных тестов за последние сутки
        $testsLast24h = TestResult::whereHas('site', function ($query) use ($userId) {
            $query->where('user_id', $userId);
        })
        ...
        $failuresLast24h = TestResult::whereHas('site', function ($query) use ($userId) {
            $query->where('user_id', $userId);
        })
        ->where('checked_at', '>=', now()->subDay())
        ->where('status', 'failed')
